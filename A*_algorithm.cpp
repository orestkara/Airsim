...
public:
    ...
    vector<Node*> findPath(int start_x, int start_y, int start_z, int goal_x, int goal_y, int goal_z)
     {
        vector<Node*> result;
            ...
            for (int dx = -100; dx <= 100; dx+=100) {
                ...
                for (int dy = -100; dy <= 100; dy+=100) {
                    ...
                    for (int dz = -100; dz <= 100; dz+=100) {
                        ...
                        if (isValid(new_x, new_y, new_z) && !isClosed(new_x, new_y, new_z)) {
...
int main()
{
    ...
    path = astar.findPath(start_x, start_y, start_z, goal_x, goal_y, goal_z);
